...
`,tt=`
    ...
    </div>`;let m="info",b;const at={info:"info",wallet:"account_balance_wallet",nft:"savings",cart:"shopping_cart",book:"import_contacts"};function j(){let t=b.indexOf(m)+1;t<b.length&&x(b[t])}function W(){let t=b.indexOf(m)-1;t>=0&&x(b[t])}function x(t){let e=document.getElementById("btn-".concat(m));e==null||e.classList.remove("gold"),e==null||e.classList.add("gold-dark"),m=t,J();let n=document.getElementById("btn-".concat(t));n==null||n.classList.remove("gold-dark"),n==null||n.classList.add("gold")}function U(t={...at}){localStorage.getItem("wallet_address")||delete t.nft,b=Object.keys(t);const e=document.getElementById("bottom-navigation");e.innerHTML="",b.forEach(n=>{let s="btn-".concat(n);e.insertAdjacentHTML("beforeend",`
            <div id="${s}" class="nav-img gold-dark" style="width: ${100/b.length}%">
                    <span class="material-symbols-outlined" >${t[n]}</span>

            </div>`),document.querySelector("#"+s).addEventListener("click",()=>{x(n)})}),x(m)}let f=window.Telegram.WebApp;function st(){f.isVerticalSwipesEnabled=!1,f.ready(),f.expand(),f.disableVerticalSwipes()}function lt(){return f.initData.trim()===""&&!k.tg_mode?(x("error_tg"),1):0}const d=new TON_CONNECT_UI.TonConnectUI({manifestUrl:"https://static.biptoken.xyz/MANIFEST.json",buttonRootId:"ton-connect",enableAndroidBackHandler:!0});d.uiOptions={twaReturnUrl:"https://t.me/BIPapp_bot",language:"ru",uiPreferences:{theme:"SYSTEM"}};d.onStatusChange(t=>{if(d.connected){const e=d.account.address;localStorage.setItem("wallet_address",e),k.tg_mode||axios.post(`https://api.biptoken.xyz/v1/auth/nuw?wallet_address=${e}&tgInitData=${encodeURIComponent(f.initData)}`)}else localStorage.setItem("wallet_address","");localStorage.setItem("wallet_connect",d.connected),G(),K(),U()});async function it(){try{await d.openModal()}catch{console.log("Invalid connect wallet")}U()}async function ot(){await d.disconnect(),U()}let u={can:!1,token:void 0,amount:0};function T(t,e){let n=document.getElementById("btn_send_transaction");localStorage.setItem(`${e}_input_count`,"0");let s;if(!t||t<=0)n&&e!=="RUB"&&(n==null||n.classList.add("opacity-2"),n.value="Введите значение больше нуля"),s="0 BIPCoin",u.can=!1;else if(isNaN(t*1))n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Введено некорректное значение"),s="Некорректное значение",u.can=!1;else{s=`${ct(t,e)} BIPCoin`,localStorage.setItem(`${e}_input_count`,t);let a=localStorage.getItem(`${e}_balance`);t*1<=a?(n&&e!=="RUB"&&(n.classList.remove("opacity-2"),n.value=`Оплатить ${t*1} ${e}`),u.can=!0,u.token=e,u.amount=t):(u.can=!1,n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Недостаточно средств на кошельке"))}try{document.getElementById(`${e}_value`).innerHTML=s}catch{}dt(t,e)}function ct(t,e){let n=localStorage.getItem("BIP_price");return e.toUpperCase()==="TON"&&(t*=localStorage.getItem("TON_price")),e.toUpperCase()==="RUB"&&(t/=localStorage.getItem("RUB_price")),Math.round(t/n*100)/100}function dt(t,e){let n;t.length<=1?n="1rem":n=`${t.length*.5}rem`,document.getElementById(`${e}_input_count`).style.width=n}function rt(){_("send_to_crypto"),_("send_to_card");let t=document.getElementById("send_to_crypto_content"),e;if(!(d.connected||k.wallet_connect==="true"))e=t.innerHTML,t.innerHTML=z;else{e&&(t.innerHTML=e);let a=document.getElementById("TON_input_count");a.addEventListener("input",i=>{T(i.target.value,"TON")});let l=document.getElementById("USDT_input_count");l.addEventListener("input",i=>{T(i.target.value,"USDT")}),L||(l.type="text",a.type="text")}let n=document.getElementById("send_to_card_content"),s;if(!(d.connected||k.wallet_connect==="true"))s=n.innerHTML,n.innerHTML=z;else{s&&(n.innerHTML=s);let a=document.getElementById("RUB_input_count");a.addEventListener("input",i=>{T(i.target.value,"RUB")}),L||(a.type="text");let l;try{l=f.initDataUnsafe.user.id}catch{l="UserID"}document.getElementById("card_send_code").innerHTML=l,document.getElementById("admin_card_number").innerHTML=h.card}}let w={can:!1,amount:0};function K(){const t=document.getElementsByClassName("wallet_please_connect"),e=document.getElementsByClassName("wallet_access_connect");if(d.connected||k.wallet_connect==="true"){for(let a of t)a.classList.add("hide");for(let a of e)a.classList.remove("hide")}else{for(let a of t)a.classList.remove("hide");for(let a of e)a.classList.add("hide")}let n=document.getElementById("BIP_transfer_input_count"),s=document.getElementById("btn_send_transaction_to_sw");n==null||n.addEventListener("input",a=>{T(a.target.value,"BIP_transfer"),Number(a.target.value)<=g.balance&&a.target.value&&a.target.value!=="0"?s.classList.remove("opacity-2"):s.classList.add("opacity-2")}),!L&&n&&(n.type="text"),M(),_t(),pt()}function pt(){try{let t=g.balance;t&&(document.getElementById("sw_balance_num").innerHTML=Math.floor(t*1e3)/1e3)}catch(t){console.log(t)}}function _t(){try{let t=document.getElementById("sw_transactions_list");if(t&&g.transactions){let e=Object.keys(g.transactions).sort(),n="",s="",a="";for(const l of e.reverse()){let i=g.transactions[l],c=new Date(l*1e3).toLocaleString("ru-RU",{month:"long",day:"numeric"});a!==c&&(a=c,n+=s,s=`<p class="text-lg">${c}</p>`);let r=i.value>0;s+=`
                <div class="m-2 ml-4">
                    <p class="inline">${i.text}</p>
                    <p class="fl-r inline ${r?"green":""}">${r?"+":"-"}${Math.round(i.value*1e3)/1e3} BIP</p>
                </div>`}n+=`<div class="mt-4 trs-day">${s}</div>`,t.innerHTML!==n&&(t.innerHTML=n)}}catch(t){console.log(t)}}function M(){let t=localStorage.getItem("BIP_balance");const e=document.getElementById("BIP_balance");let n=localStorage.getItem("BIP_price");const s=document.getElementById("BIP_price");let a=localStorage.getItem("TON_balance");const l=document.getElementById("TON_balance");let i=localStorage.getItem("USDT_balance");const p=document.getElementById("USDT_balance");e&&(e.innerHTML=Math.floor(t*100)/100,l.innerHTML=Math.floor(a*100)/100,p.innerHTML=Math.floor(i*100)/100,s.innerHTML=Math.floor(n*100)/100)}let E=null;function mt(){document.addEventListener("touchstart",ut,!1),document.addEventListener("touchmove",gt,!1),document.addEventListener("keydown",t=>{t.key==="ArrowRight"&&j(),t.key==="ArrowLeft"&&W()})}function ut(t){E=t.touches[0].clientX,t.touches[0].clientY}function gt(t){if(!E)return;const e=t.touches[0].clientX;t.touches[0].clientY;const n=E-e;Math.abs(n)>window.screen.width/5&&(n>0?j():n<0&&W(),E=null)}const v=new window.TonWeb,y=v.utils.Address;async function V(t,e=!0){let n=new v.boc.Cell;return n.bits.writeUint(0,32),n.bits.writeString(t||""),e?v.utils.bytesToBase64(await n.toBoc()):n}async function ft(t,e,n=1,s=null){let a=new v.boc.Cell;return a.bits.writeUint(260734629,32),a.bits.writeUint(Date.now(),64),a.bits.writeCoins(t),a.bits.writeAddress(new y(e)),a.bits.writeAddress(new y(localStorage.getItem("wallet_address"))),a.bits.writeUint(0,1),a.bits.writeCoins(n),s?(a.bits.writeUint(1,1),a.refs.push(V(s,!1))):a.bits.writeUint(0,1),v.utils.bytesToBase64(await a.toBoc())}async function bt(t,e=.15*1e9){let n=new v.boc.Cell;return n.bits.writeUint(1607220500,32),n.bits.writeUint(Date.now(),64),n.bits.writeAddress(new y(t)),n.bits.writeAddress(new y(localStorage.getItem("wallet_address"))),n.bits.writeUint(0,1),n.bits.writeCoins(e),n.bits.writeUint(0,1),v.utils.bytesToBase64(await n.toBoc())}async function wt(t){const e=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:e,amount:(.2*1e9).toString(),payload:await bt(h.nft_wallet)}]}}async function Q(t,e,n=null){const s=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:s,amount:(e*1e9).toString(),payload:await V(n)}]}}async function X(t,e,n,s=1e-9){const a=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:a,amount:((.06+s)*1e9).toString(),payload:await ft(n*1e9,e,s*1e9)}]}}async function vt(t,e){let n;if(t==="TON"?n=Q(h.buy_wallet,e):t==="USDT"&&(n=X(localStorage.getItem("USDT_wallet_address"),h.buy_wallet,e*.001)),n)return await d.sendTransaction(await n)}async function yt(t){return await d.sendTransaction(await X(localStorage.getItem("BIP_wallet_address"),h.nft_wallet,t,.15))}async function ht(t,e,n=null){return await d.sendTransaction(await Q(t,e,n))}async function It(t){return await d.sendTransaction(await wt(t))}async function Bt(){document.addEventListener("click",async function(t){var e,n,s,a,l,i,p,c,r;if((e=t.target)!=null&&e.className.includes("btn_page_wallet")&&x("wallet"),(n=t.target)!=null&&n.className.includes("btn_wallet_connect")&&await it(),(s=t.target)!=null&&s.className.includes("btn_wallet_disconnect")&&await ot(),((a=t.target)==null?void 0:a.id)==="btn_send_transaction"&&u.can&&(await vt(u.token,u.amount)?document.getElementById("btn_send_transaction").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction").value="Ошибка при отправке транзакции"),((l=t.target)==null?void 0:l.id)==="btn_send_transaction_to_nft"&&w.can&&(await yt(w.amount)?document.getElementById("btn_send_transaction_to_nft").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_nft").value="Ошибка при отправке транзакции"),((i=t.target)==null?void 0:i.id)==="btn_send_transaction_to_sw"){let o=Number(localStorage.getItem("BIP_transfer_input_count"));o<=g.balance&&(await ht(h.sw_cash_out,.05,o)?document.getElementById("btn_send_transaction_to_sw").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_sw").value="Ошибка при отправке транзакции")}if(((p=t.target)==null?void 0:p.id)==="copy_admin_card_number"){let o=document.getElementById("admin_card_number");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((c=t.target)==null?void 0:c.id)==="copy_card_send_code"){let o=document.getElementById("card_send_code");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((r=t.target)==null?void 0:r.classList.toString().split(" ").indexOf("BIP_NFT_sale_button"))!==-1)for(const o of P)o.index.toString()===t.target.id.split("_")[1]&&(console.log(o.address),console.log(P),await It(o.address))})}let S=0;function xt(){if(Number(localStorage.getItem("BIP_balance"))>0){let t=document.getElementById("BIP_input_count");t==null||t.addEventListener("input",e=>{T(e.target.value,"BIP")}),!L&&t&&(t.type="text")}O()}function O(){var t,e,n,s,a;try{if(localStorage.getItem("BIP_balance")>0?((t=document.getElementById("BIP_input_count"))==null||t.addEventListener("input",l=>{kt(l.target.value)}),(e=document.getElementById("changer_BIP2NFT"))==null||e.classList.remove("hide")):(n=document.getElementById("changer_BIP2NFT"))==null||n.classList.add("hide"),P.length!==0){let l=document.getElementById("wallet_nft_list"),i=1,p="";P.forEach(c=>{let r=c.metadata.name;r==="BIP NFT"&&(r=c.metadata.attributes[0].value.toString()+" "+r);let o=`BIPNFT_${c.index}`;p+=`
