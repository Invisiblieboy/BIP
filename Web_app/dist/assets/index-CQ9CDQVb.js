...
`,Z=`
    ...
    <div id = "error-tg">
        ...
        <div class="message-cloud mt-8">
    ...
    </div>`;let m="info",b;const nt={info:"info",wallet:"account_balance_wallet",nft:"savings",cart:"shopping_cart",book:"import_contacts"};function A(){let t=b.indexOf(m)+1;t<b.length&&B(b[t])}function j(){let t=b.indexOf(m)-1;t>=0&&B(b[t])}function B(t){let e=document.getElementById("btn-".concat(m));e==null||e.classList.remove("gold"),e==null||e.classList.add("gold-dark"),m=t,G();let n=document.getElementById("btn-".concat(t));n==null||n.classList.remove("gold-dark"),n==null||n.classList.add("gold")}function E(t={...nt}){localStorage.getItem("wallet_address")||delete t.nft,b=Object.keys(t);const e=document.getElementById("bottom-navigation");e.innerHTML="",b.forEach(n=>{let s="btn-".concat(n);e.insertAdjacentHTML("beforeend",`
            <div id="${s}" class="nav-img gold-dark" style="width: ${100/b.length}%">
                    <span class="material-symbols-outlined" >${t[n]}</span>

            </div>`),document.querySelector("#"+s).addEventListener("click",()=>{B(n)})}),B(m)}let f=window.Telegram.WebApp;function at(){f.isVerticalSwipesEnabled=!1,f.ready(),f.expand(),f.disableVerticalSwipes()}function st(){return f.initData.trim()===""&&!x.tg_mode?(B("error_tg"),1):0}const d=new TON_CONNECT_UI.TonConnectUI({manifestUrl:"https://static.biptoken.xyz/MANIFEST.json",buttonRootId:"ton-connect",enableAndroidBackHandler:!0});d.uiOptions={twaReturnUrl:"https://t.me/BIPapp_bot",language:"ru",uiPreferences:{theme:"SYSTEM"}};d.onStatusChange(t=>{if(d.connected){const e=d.account.address;localStorage.setItem("wallet_address",e),x.tg_mode||axios.post(`https://api.biptoken.xyz/v1/auth/nuw?wallet_address=${e}&tgInitData=${encodeURIComponent(f.initData)}`)}else localStorage.setItem("wallet_address","");localStorage.setItem("wallet_connect",d.connected),X(),W(),E()});async function lt(){try{await d.openModal()}catch{console.log("Invalid connect wallet")}E()}async function it(){await d.disconnect(),E()}let u={can:!1,token:void 0,amount:0};function k(t,e){let n=document.getElementById("btn_send_transaction");localStorage.setItem(`${e}_input_count`,"0");let s;if(!t||t<=0)n&&e!=="RUB"&&(n==null||n.classList.add("opacity-2"),n.value="Введите значение больше нуля"),s="0 BIPCoin",u.can=!1;else if(isNaN(t*1))n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Введено некорректное значение"),s="Некорректное значение",u.can=!1;else{s=`${ot(t,e)} BIPCoin`,localStorage.setItem(`${e}_input_count`,t);let a=localStorage.getItem(`${e}_balance`);t*1<=a?(n&&e!=="RUB"&&(n.classList.remove("opacity-2"),n.value=`Оплатить ${t*1} ${e}`),u.can=!0,u.token=e,u.amount=t):(u.can=!1,n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Недостаточно средств на кошельке"))}try{document.getElementById(`${e}_value`).innerHTML=s}catch{}ct(t,e)}function ot(t,e){let n=localStorage.getItem("BIP_price");return e.toUpperCase()==="TON"&&(t*=localStorage.getItem("TON_price")),e.toUpperCase()==="RUB"&&(t/=localStorage.getItem("RUB_price")),Math.round(t/n*100)/100}function ct(t,e){let n;t.length<=1?n="1rem":n=`${t.length*.5}rem`,document.getElementById(`${e}_input_count`).style.width=n}function dt(){_("send_to_crypto"),_("send_to_card");let t=document.getElementById("send_to_crypto_content"),e;if(!(d.connected||x.wallet_connect==="true"))e=t.innerHTML,t.innerHTML=q;else{e&&(t.innerHTML=e);let a=document.getElementById("TON_input_count");a.addEventListener("input",i=>{k(i.target.value,"TON")});let l=document.getElementById("USDT_input_count");l.addEventListener("input",i=>{k(i.target.value,"USDT")}),P||(l.type="text",a.type="text")}let n=document.getElementById("send_to_card_content"),s;if(!(d.connected||x.wallet_connect==="true"))s=n.innerHTML,n.innerHTML=q;else{s&&(n.innerHTML=s);let a=document.getElementById("RUB_input_count");a.addEventListener("input",i=>{k(i.target.value,"RUB")}),P||(a.type="text");let l;try{l=f.initDataUnsafe.user.id}catch{l="UserID"}document.getElementById("card_send_code").innerHTML=l,document.getElementById("admin_card_number").innerHTML=h.card}}let w={can:!1,amount:0};function W(){const t=document.getElementsByClassName("wallet_please_connect"),e=document.getElementsByClassName("wallet_access_connect");if(d.connected||x.wallet_connect==="true"){for(let a of t)a.classList.add("hide");for(let a of e)a.classList.remove("hide")}else{for(let a of t)a.classList.remove("hide");for(let a of e)a.classList.add("hide")}let n=document.getElementById("BIP_transfer_input_count"),s=document.getElementById("btn_send_transaction_to_sw");n==null||n.addEventListener("input",a=>{k(a.target.value,"BIP_transfer"),Number(a.target.value)<=g.balance&&a.target.value&&a.target.value!=="0"?s.classList.remove("opacity-2"):s.classList.add("opacity-2")}),!P&&n&&(n.type="text"),C(),pt(),rt()}function rt(){try{let t=g.balance;t&&(document.getElementById("sw_balance_num").innerHTML=Math.floor(t*1e3)/1e3)}catch(t){console.log(t)}}function pt(){try{let t=document.getElementById("sw_transactions_list");if(t&&g.transactions){let e=Object.keys(g.transactions).sort(),n="",s="",a="";for(const l of e.reverse()){let i=g.transactions[l],c=new Date(l*1e3).toLocaleString("ru-RU",{month:"long",day:"numeric"});a!==c&&(a=c,n+=s,s=`<p class="text-lg">${c}</p>`);let r=i.value>0;s+=`
                <div class="m-2 ml-4">
                    <p class="inline">${i.text}</p>
                    <p class="fl-r inline ${r?"green":""}">${r?"+":"-"}${Math.round(i.value*1e3)/1e3} BIP</p>
                </div>`}n+=`<div class="mt-4 trs-day">${s}</div>`,t.innerHTML!==n&&(t.innerHTML=n)}}catch(t){console.log(t)}}function C(){let t=localStorage.getItem("BIP_balance");const e=document.getElementById("BIP_balance");let n=localStorage.getItem("BIP_price");const s=document.getElementById("BIP_price");let a=localStorage.getItem("TON_balance");const l=document.getElementById("TON_balance");let i=localStorage.getItem("USDT_balance");const p=document.getElementById("USDT_balance");e&&(e.innerHTML=Math.floor(t*100)/100,l.innerHTML=Math.floor(a*100)/100,p.innerHTML=Math.floor(i*100)/100,s.innerHTML=Math.floor(n*100)/100)}let N=null;function _t(){document.addEventListener("touchstart",mt,!1),document.addEventListener("touchmove",ut,!1),document.addEventListener("keydown",t=>{t.key==="ArrowRight"&&A(),t.key==="ArrowLeft"&&j()})}function mt(t){N=t.touches[0].clientX,t.touches[0].clientY}function ut(t){if(!N)return;const e=t.touches[0].clientX;t.touches[0].clientY;const n=N-e;Math.abs(n)>window.screen.width/5&&(n>0?A():n<0&&j(),N=null)}const v=new window.TonWeb,y=v.utils.Address;async function K(t,e=!0){let n=new v.boc.Cell;return n.bits.writeUint(0,32),n.bits.writeString(t||""),e?v.utils.bytesToBase64(await n.toBoc()):n}async function gt(t,e,n=1,s=null){let a=new v.boc.Cell;return a.bits.writeUint(260734629,32),a.bits.writeUint(Date.now(),64),a.bits.writeCoins(t),a.bits.writeAddress(new y(e)),a.bits.writeAddress(new y(localStorage.getItem("wallet_address"))),a.bits.writeUint(0,1),a.bits.writeCoins(n),s?(a.bits.writeUint(1,1),a.refs.push(K(s,!1))):a.bits.writeUint(0,1),v.utils.bytesToBase64(await a.toBoc())}async function ft(t,e=.15*1e9){let n=new v.boc.Cell;return n.bits.writeUint(1607220500,32),n.bits.writeUint(Date.now(),64),n.bits.writeAddress(new y(t)),n.bits.writeAddress(new y(localStorage.getItem("wallet_address"))),n.bits.writeUint(0,1),n.bits.writeCoins(e),n.bits.writeUint(0,1),v.utils.bytesToBase64(await n.toBoc())}async function bt(t){const e=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:e,amount:(.2*1e9).toString(),payload:await ft(h.nft_wallet)}]}}async function V(t,e,n=null){const s=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:s,amount:(e*1e9).toString(),payload:await K(n)}]}}async function Q(t,e,n,s=1e-9){const a=new y(t).toString(!0,!0,!0);return{validUntil:Math.floor(new Date/1e3)+4*60,messages:[{address:a,amount:((.06+s)*1e9).toString(),payload:await gt(n*1e9,e,s*1e9)}]}}async function wt(t,e){let n;if(t==="TON"?n=V(h.buy_wallet,e):t==="USDT"&&(n=Q(localStorage.getItem("USDT_wallet_address"),h.buy_wallet,e*.001)),n)return await d.sendTransaction(await n)}async function vt(t){return await d.sendTransaction(await Q(localStorage.getItem("BIP_wallet_address"),h.nft_wallet,t,.15))}async function yt(t,e,n=null){return await d.sendTransaction(await V(t,e,n))}async function ht(t){return await d.sendTransaction(await bt(t))}async function It(){document.addEventListener("click",async function(t){var e,n,s,a,l,i,p,c,r;if((e=t.target)!=null&&e.className.includes("btn_page_wallet")&&B("wallet"),(n=t.target)!=null&&n.className.includes("btn_wallet_connect")&&await lt(),(s=t.target)!=null&&s.className.includes("btn_wallet_disconnect")&&await it(),((a=t.target)==null?void 0:a.id)==="btn_send_transaction"&&u.can&&(await wt(u.token,u.amount)?document.getElementById("btn_send_transaction").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction").value="Ошибка при отправке транзакции"),((l=t.target)==null?void 0:l.id)==="btn_send_transaction_to_nft"&&w.can&&(await vt(w.amount)?document.getElementById("btn_send_transaction_to_nft").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_nft").value="Ошибка при отправке транзакции"),((i=t.target)==null?void 0:i.id)==="btn_send_transaction_to_sw"){let o=Number(localStorage.getItem("BIP_transfer_input_count"));o<=g.balance&&(await yt(h.sw_cash_out,.05,o)?document.getElementById("btn_send_transaction_to_sw").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_sw").value="Ошибка при отправке транзакции")}if(((p=t.target)==null?void 0:p.id)==="copy_admin_card_number"){let o=document.getElementById("admin_card_number");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((c=t.target)==null?void 0:c.id)==="copy_card_send_code"){let o=document.getElementById("card_send_code");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((r=t.target)==null?void 0:r.classList.toString().split(" ").indexOf("BIP_NFT_sale_button"))!==-1)for(const o of T)o.index.toString()===t.target.id.split("_")[1]&&(console.log(o.address),console.log(T),await ht(o.address))})}let L=0;function Bt(){if(Number(localStorage.getItem("BIP_balance"))>0){let t=document.getElementById("BIP_input_count");t==null||t.addEventListener("input",e=>{k(e.target.value,"BIP")}),!P&&t&&(t.type="text")}M()}function M(){var t,e,n,s,a;try{if(localStorage.getItem("BIP_balance")>0?((t=document.getElementById("BIP_input_count"))==null||t.addEventListener("input",l=>{xt(l.target.value)}),(e=document.getElementById("changer_BIP2NFT"))==null||e.classList.remove("hide")):(n=document.getElementById("changer_BIP2NFT"))==null||n.classList.add("hide"),T.length!==0){let l=document.getElementById("wallet_nft_list"),i=1,p="";T.forEach(c=>{let r=c.metadata.name;r==="BIP NFT"&&(r=c.metadata.attributes[0].value.toString()+" "+r);let o=`BIPNFT_${c.index}`;p+=`
            ...
            `,i++}),l&&(l==null?void 0:l.innerHTML)!==p&&(l.innerHTML=p),(s=document.getElementById("changer_NFT2BIP"))==null||s.classList.remove("hide")}else(a=document.getElementById("changer_NFT2BIP"))==null||a.classList.add("hide")}catch(l){console.log(l),setTimeout(M,500)}}function xt(t){let e=document.getElementById("btn_send_transaction_to_nft");if(localStorage.setItem("BIP_input_count","0"),!t||t<=0)e&&(e==null||e.classList.add("opacity-2"),e.value="Введите значение больше нуля"),document.getElementById("BIP_input_count").innerHTML="0 BIPCoin",w.can=!1,U();else if(isNaN(t*1))e&&(e.classList.add("opacity-2"),e.value="Некорректное значение"),document.getElementById("BIP_input_count").innerHTML="Некорректное значение",w.can=!1,U();else{document.getElementById("BIP_input_count").innerHTML=`${t} BIPCoin`,localStorage.setItem("BIP_input_count",t),U(t);let s=localStorage.getItem("BIP_balance");t*1<=s?(e&&(e.classList.remove("opacity-2"),e.value="Получить NFT"),w.can=!0,w.amount=t):(w.can=!1,e&&(e.classList.add("opacity-2"),e.value="Недостаточно средств"))}let n;t.length<=1?n="1rem":n=`${t.length*.5}rem`,document.getElementById("BIP_input_count").style.width=n}function U(t=0){let e=document.getElementById("nft_preshow_img");if(t<=0){e.classList.add("hide");return}L===t||new Array(e.classList).toString().split(" ").indexOf("hide")===-1&&(t>=1e4&&L>=1e4||t<=99&&L<=99)||(L=t,axios.get(`https://nft.biptoken.xyz/CreateItemMetadata?value=${t}`,{responseType:"json",transformResponse:s=>s}).then(s=>{let a=document.getElementById("wallet_nft_items").scrollHeight;e.style.height=`${a+Ct(1)-2}px`;let l=JSON.parse(s.data).image;l!==e.src&&(e.src=l),e.classList.remove("hide")}).catch(s=>{console.error(`Got a bad format response with status code ${s.response.status}: ${s.response.data}`)}))}let x={},T=[],h,P,g={};const kt="0:b113a994b5024a16719f69139328eb759596c38a25f59028b146fecdc3621dfe",Tt="0:786e74bba071d57c75004bb41bf978ea73e00b03f3f3fbf2b73ea0426f97d6e9",F={BIP:Tt,USDT:kt};function Pt(){let t=location.search.substring(1).split("&");for(let e=0;e<t.length;e++){let n=t[e].split("=");x[n[0]]=n[1]}}function Lt(t=f.initData){if(!t)return g;axios.get("https://api.biptoken.xyz/v1/wallet/info",{params:{tgInitData:t}}).then(e=>{e.data.op==="0"&&(g=e.data.data)}).finally(()=>g)}function St(t,e,...n){return t(...n),setInterval(t,e,...n)}function Nt(){axios.get("https://www.cbr-xml-daily.ru/daily_json.js").then(t=>{localStorage.setItem("RUB_price",t.data.Valute.USD.Value)}).finally(()=>localStorage.getItem("RUB_price"))}function Et(){axios.get("https://api.coingecko.com/api/v3/coins/the-open-network").then(t=>{localStorage.setItem("TON_price",t.data.market_data.current_price.usd)}).finally(()=>localStorage.getItem("TON_price"))}function Ut(){axios.get("https://api.geckoterminal.com/api/v2/networks/ton/pools/EQCJWbehGyHtkgw6IMzcY1VPAJwre1sWWGvqKYm1_h0CpsJe").then(e=>{localStorage.setItem("BIP_price",e.data.data.attributes.base_token_price_quote_token)}).finally(()=>localStorage.getItem("BIP_price"))}function X(){let t=localStorage.getItem("wallet_address");if(t)axios.get(`https://tonapi.io/v2/accounts/${t}/jettons`).then(e=>{axios.get(`https://tonapi.io/v2/accounts/${localStorage.getItem("wallet_address")}/nfts?collection=EQDhO_YVxvb3p68hqte0kNG5jdO44WQFgWdCe8GvgVkrws4Z`).then(n=>{T=n.data.nft_items,M()});for(let n of e.data.balances)for(let[s,a]of Object.entries(F))n.jetton.address===a&&(localStorage.setItem(`${s}_balance`,n.balance/10**n.jetton.decimals),localStorage.setItem(`${s}_wallet_address`,n.wallet_address.address));C()}),axios.get(`https://tonapi.io/v2/accounts/${t}`).then(e=>{localStorage.setItem("TON_balance",e.data.balance/10**9)}).catch(e=>{localStorage.setItem("TON_balance",0)}).finally(()=>{C()});else{localStorage.setItem("TON_balance",0);for(let[e,n]of Object.entries(F))localStorage.setItem(`${e}_balance`,"0"),localStorage.setItem(`${e}_wallet_address`,"")}}function Ct(t){return t*parseFloat(getComputedStyle(document.documentElement).fontSize)}let S={};function _(t){let e=document.getElementById(`${t}_arrow`),n=document.getElementById(`${t}_content`);S[t]=0,e.addEventListener("click",()=>{S[t]?(e.innerHTML="keyboard_arrow_down",n.classList.add("hide"),S[t]=0):(e.innerHTML="keyboard_arrow_up",n.classList.remove("hide"),S[t]=1)})}async function Mt(){await It(),at(),_t(),document.querySelectorAll(".material-symbols-outlined").forEach(n=>{n.classList.add("icon-load")}),document.fonts.ready.then(()=>{document.querySelectorAll(".material-symbols-outlined").forEach(n=>{n.classList.remove("icon-load")})});const t=document.querySelector("#root"),e=document.createElement("div");e.id="bottom-navigation",t.appendChild(e),setInterval(Nt,20*60*1e3),setInterval(Et,60*1e3),setInterval(Ut,60*1e3),St(Lt,10*1e3),setInterval(X,10*1e3),await axios.get("https://api.biptoken.xyz/v1/info/payments").then(n=>{n.status===200&&(h=n.data)}),P=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}async function Ot(){document.addEventListener("click",async function(t){var e,n,s,a,l,i,p,c,r,o,O,$,H,D;(e=t.target)!=null&&e.className.includes("link-biptoken")&&window.open("https://t.me/BIPholders","_blank"),(n=t.target)!=null&&n.className.includes("link-tg-how-to-buy")&&window.open("https://t.me/BIPholders/6","_blank"),(s=t.target)!=null&&s.className.includes("link-bipizm")&&window.open("https://t.me/BIPuserchat","_blank"),(a=t.target)!=null&&a.className.includes("link-biplions")&&window.open("https://t.me/biplions1","_blank"),(l=t.target)!=null&&l.className.includes("link-tgbot")&&window.open("https://t.me/BIPapp_bot?start=app","_blank"),(i=t.target)!=null&&i.className.includes("link-baltinvestproject")&&window.open("https://t.me/BaltInvestProekt","_blank"),(p=t.target)!=null&&p.className.includes("link-how-buy-usdt-on-wallet")&&window.open("https://telegra.ph/Kak-kupit-BIP-imeya-USDT-ili-TON-ili-lyuboj-drugoj-token-12-04","_blank"),(c=t.target)!=null&&c.className.includes("link-how-buy-usdt-on-CEX")&&window.open("https://telegra.ph/Kak-kupit-BIP-imeya-USDT-ili-TON-na-Birzhe-12-04","_blank"),(r=t.target)!=null&&r.className.includes("link-how-buy-verif-on-CEX")&&window.open("https://telegra.ph/Kak-kupit-BIP-imeya-verifikaciyu-na-Birzhe-12-04","_blank"),(o=t.target)!=null&&o.className.includes("link-how-buy-rub")&&window.open("https://telegra.ph/Kak-kupit-BIP-imeya-RUB-12-04","_blank"),(O=t.target)!=null&&O.className.includes("link-how-sell-rub")&&window.open("https://telegra.ph/Kak-prodat-BIPCoin-02-08","_blank"),($=t.target)!=null&&$.className.includes("link-how-create-tk")&&window.open("https://telegra.ph/Kak-sozdat-svoj-koshelek-Tonkeeper-12-16","_blank"),(H=t.target)!=null&&H.className.includes("link-baltinvestpro-stats")&&window.open("https://www.rusprofile.ru/id/4405609","_blank"),(D=t.target)!=null&&D.className.includes("link-NFT-lock-500k")&&window.open("https://tonviewer.com/EQAMKtgKB4QVDFx5Krl4Z5QM8-j5UULNaHGcZHJRjIf4o1SL","_blank")})}function R(){_("faq"),_("faq_get_bip"),_("faq_why_we_work"),_("faq_how_bip_work"),_("faq_how_much_pay"),_("faq_what_we_suggest"),_("faq_why_30_percents"),_("faq_how_much_total_BIPCoin"),_("faq_details"),document.getElementById("faq_arrow").addEventListener("click",()=>{document.getElementById("faq").classList.toggle("inline")})}const I=document.querySelector("#page");function G(){m==="info"?(I.innerHTML=z,R()):m==="wallet"?(I.innerHTML=J,W()):m==="nft"?(I.innerHTML=Y,Bt()):m==="cart"?(I.innerHTML=Z,dt()):m==="book"?I.innerHTML=tt:m==="error_tg"&&(I.innerHTML=z+et,R())}async function $t(){Pt(),await Ot(),st()||(await Mt(),E(),G())}(async()=>document.addEventListener("DOMContentLoaded",async()=>$t()))();
