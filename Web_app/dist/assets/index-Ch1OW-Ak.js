        ...
        <div class="message-cloud inline text-sm" id="faq">
            ...
            <div class="hide mt-3" id="faq_content">
                ...
                <div class="hide ml-4 mt-1" id="faq_how_bip_work_content">
                    ...
                    Продать BIPCoin Вы можете в любой момент через <span class="site-link link-how-sell-rub">dedust</span>.
                    </p>
                </div>
                ...
                <div class="hide ml-4 mt-1" id="faq_how_much_total_BIPCoin_content">
                    <p>
                    Общее количество токенов - 1 млн, но 500 000 мы специально заморозим на год. Кроме того, таким образом мы создаем самоподдерживающуюся систему, так как должны выкупать токены для обеспечения выплат.
                    </p>
                </div>
...
`,Q=`
    ...
    </div>`;let m="info",b;const tt={info:"info",wallet:"account_balance_wallet",nft:"savings",cart:"shopping_cart",book:"import_contacts"};function z(){let t=b.indexOf(m)+1;t<b.length&&h(b[t])}function R(){let t=b.indexOf(m)-1;t>=0&&h(b[t])}function h(t){let e=document.getElementById("btn-".concat(m));e==null||e.classList.remove("gold"),e==null||e.classList.add("gold-dark"),m=t,X();let n=document.getElementById("btn-".concat(t));n==null||n.classList.remove("gold-dark"),n==null||n.classList.add("gold")}function N(t={...tt}){localStorage.getItem("wallet_address")||delete t.nft,b=Object.keys(t);const e=document.getElementById("bottom-navigation");e.innerHTML="",b.forEach(n=>{let a="btn-".concat(n);e.insertAdjacentHTML("beforeend",`
            <div id="${a}" class="nav-img gold-dark" style="width: ${100/b.length}%">
                    <span class="material-symbols-outlined" >${t[n]}</span>

            </div>`),document.querySelector("#"+a).addEventListener("click",()=>{h(n)})}),h(m)}let f=window.Telegram.WebApp;function et(){f.isVerticalSwipesEnabled=!1,f.ready(),f.expand(),f.disableVerticalSwipes()}function nt(){return f.initData.trim()===""&&!I.tg_mode?(h("error_tg"),1):0}const r=new TON_CONNECT_UI.TonConnectUI({manifestUrl:"https://static.biptoken.xyz/MANIFEST.json",buttonRootId:"ton-connect",enableAndroidBackHandler:!0});r.uiOptions={twaReturnUrl:"https://t.me/BIPapp_bot",language:"ru",uiPreferences:{theme:"SYSTEM"}};r.onStatusChange(t=>{if(r.connected){const e=r.account.address;localStorage.setItem("wallet_address",e),I.tg_mode||axios.post(`https://api.biptoken.xyz/v1/auth/nuw?wallet_address=${e}&tgInitData=${encodeURIComponent(f.initData)}`)}else localStorage.setItem("wallet_address","");localStorage.setItem("wallet_connect",r.connected),K(),F(),N()});async function at(){try{await r.openModal()}catch{console.log("Invalid connect wallet")}N()}async function st(){await r.disconnect(),N()}let u={can:!1,token:void 0,amount:0};function x(t,e){let n=document.getElementById("btn_send_transaction");localStorage.setItem(`${e}_input_count`,"0");let a;if(!t||t<=0)n&&e!=="RUB"&&(n==null||n.classList.add("opacity-2"),n.value="Введите значение больше нуля"),a="0 BIPCoin",u.can=!1;else if(isNaN(t*1))n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Введено некорректное значение"),a="Некорректное значение",u.can=!1;else{a=`${lt(t,e)} BIPCoin`,localStorage.setItem(`${e}_input_count`,t);let s=localStorage.getItem(`${e}_balance`);t*1<=s?(n&&e!=="RUB"&&(n.classList.remove("opacity-2"),n.value=`Оплатить ${t*1} ${e}`),u.can=!0,u.token=e,u.amount=t):(u.can=!1,n&&e!=="RUB"&&(n.classList.add("opacity-2"),n.value="Недостаточно средств на кошельке"))}try{document.getElementById(`${e}_value`).innerHTML=a}catch{}it(t,e)}function lt(t,e){let n=localStorage.getItem("BIP_price");return e.toUpperCase()==="TON"&&(t*=localStorage.getItem("TON_price")),e.toUpperCase()==="RUB"&&(t/=localStorage.getItem("RUB_price")),Math.round(t/n*100)/100}function it(t,e){let n;t.length<=1?n="1rem":n=`${t.length*.5}rem`,document.getElementById(`${e}_input_count`).style.width=n}function ot(){_("send_to_crypto"),_("send_to_card");let t=document.getElementById("send_to_crypto_content"),e;if(!(r.connected||I.wallet_connect==="true"))e=t.innerHTML,t.innerHTML=q;else{e&&(t.innerHTML=e);let s=document.getElementById("TON_input_count");s.addEventListener("input",i=>{x(i.target.value,"TON")});let l=document.getElementById("USDT_input_count");l.addEventListener("input",i=>{x(i.target.value,"USDT")}),T||(l.type="text",s.type="text")}let n=document.getElementById("send_to_card_content"),a;if(!(r.connected||I.wallet_connect==="true"))a=n.innerHTML,n.innerHTML=q;else{a&&(n.innerHTML=a);let s=document.getElementById("RUB_input_count");s.addEventListener("input",i=>{x(i.target.value,"RUB")}),T||(s.type="text");let l;try{l=f.initDataUnsafe.user.id}catch{l="UserID"}document.getElementById("card_send_code").innerHTML=l,document.getElementById("admin_card_number").innerHTML=B.card}}let v={can:!1,amount:0};function F(){const t=document.getElementsByClassName("wallet_please_connect"),e=document.getElementsByClassName("wallet_access_connect");if(r.connected||I.wallet_connect==="true"){for(let s of t)s.classList.add("hide");for(let s of e)s.classList.remove("hide")}else{for(let s of t)s.classList.remove("hide");for(let s of e)s.classList.add("hide")}let n=document.getElementById("BIP_transfer_input_count"),a=document.getElementById("btn_send_transaction_to_sw");n==null||n.addEventListener("input",s=>{x(s.target.value,"BIP_transfer"),Number(s.target.value)<=g.balance&&s.target.value&&s.target.value!=="0"?a.classList.remove("opacity-2"):a.classList.add("opacity-2")}),!T&&n&&(n.type="text"),C(),dt(),ct()}function ct(){try{let t=g.balance;t&&(document.getElementById("sw_balance_num").innerHTML=Math.floor(t*1e3)/1e3)}catch(t){console.log(t)}}function dt(){try{let t=document.getElementById("sw_transactions_list");if(t&&g.transactions){let e=Object.keys(g.transactions).sort(),n="",a="",s="";for(const l of e.reverse()){let i=g.transactions[l],c=new Date(l*1e3).toLocaleString("ru-RU",{month:"long",day:"numeric"});s!==c&&(s=c,n+=a,a=`<p class="text-lg">${c}</p>`);let d=i.value>0;a+=`
                <div class="m-2 ml-4">
                    <p class="inline">${i.text}</p>
                    <p class="fl-r inline ${d?"green":""}">${d?"+":"-"}${Math.round(i.value*1e3)/1e3} BIP</p>
                </div>`}n+=`<div class="mt-4 trs-day">${a}</div>`,t.innerHTML!==n&&(t.innerHTML=n)}}catch(t){console.log(t)}}function C(){let t=localStorage.getItem("BIP_balance");const e=document.getElementById("BIP_balance");let n=localStorage.getItem("BIP_price");const a=document.getElementById("BIP_price");let s=localStorage.getItem("TON_balance");const l=document.getElementById("TON_balance");let i=localStorage.getItem("USDT_balance");const p=document.getElementById("USDT_balance");e&&(e.innerHTML=Math.floor(t*100)/100,l.innerHTML=Math.floor(s*100)/100,p.innerHTML=Math.floor(i*100)/100,a.innerHTML=Math.floor(n*100)/100)}let S=null;function rt(){document.addEventListener("touchstart",pt,!1),document.addEventListener("touchmove",_t,!1),document.addEventListener("keydown",t=>{t.key==="ArrowRight"&&z(),t.key==="ArrowLeft"&&R()})}function pt(t){S=t.touches[0].clientX,t.touches[0].clientY}function _t(t){if(!S)return;const e=t.touches[0].clientX;t.touches[0].clientY;const n=S-e;Math.abs(n)>window.screen.width/5&&(n>0?z():n<0&&R(),S=null)}const w=new window.TonWeb;async function j(t,e=!0){let n=new w.boc.Cell;return n.bits.writeUint(0,32),n.bits.writeString(t||""),e?w.utils.bytesToBase64(await n.toBoc()):n}async function mt(t,e,n=1,a=null){const s=w.utils.Address;let l=new w.boc.Cell;return l.bits.writeUint(260734629,32),l.bits.writeUint(Date.now(),64),l.bits.writeCoins(t),l.bits.writeAddress(new s(e)),l.bits.writeAddress(new s(localStorage.getItem("wallet_address"))),l.bits.writeUint(0,1),l.bits.writeCoins(n),a?(l.bits.writeUint(1,1),l.refs.push(j(a,!1))):l.bits.writeUint(0,1),w.utils.bytesToBase64(await l.toBoc())}async function ut(t,e=.15*1e9){const n=w.utils.Address;let a=new w.boc.Cell;return a.bits.writeUint(1607220500,32),a.bits.writeUint(Date.now(),64),a.bits.writeAddress(new n(t)),a.bits.writeAddress(new n(localStorage.getItem("wallet_address"))),a.bits.writeUint(0,1),a.bits.writeCoins(e),a.bits.writeUint(0,1),w.utils.bytesToBase64(await a.toBoc())}async function gt(t){return{validUntil:Math.floor(new Date/1e3)+360,messages:[{address:t,amount:.2*1e9,payload:await ut(B.nft_wallet)}]}}async function W(t,e,n=null){return{validUntil:Math.floor(new Date/1e3)+360,messages:[{address:t,amount:e*1e9,payload:await j(n)}]}}async function V(t,e,n,a=1e-9){return{validUntil:Math.floor(new Date/1e3)+360,messages:[{address:t,amount:(.06+a)*1e9,payload:await mt(n*1e9,e,a)}]}}async function ft(t,e){let n;if(t==="TON"?n=W(B.buy_wallet,e):t==="USDT"&&(n=V(localStorage.getItem("USDT_wallet_address"),B.buy_wallet,e*.001)),n)return await r.sendTransaction(await n)}async function bt(t){return V(localStorage.getItem("BIP_wallet_address"),B.nft_wallet,t,.15)}async function wt(t,e,n=null){return await r.sendTransaction(await W(t,e,n))}async function vt(t){return await r.sendTransaction(await gt(t))}async function yt(){document.addEventListener("click",async function(t){var e,n,a,s,l,i,p,c,d;if((e=t.target)!=null&&e.className.includes("btn_page_wallet")&&h("wallet"),(n=t.target)!=null&&n.className.includes("btn_wallet_connect")&&await at(),(a=t.target)!=null&&a.className.includes("btn_wallet_disconnect")&&await st(),((s=t.target)==null?void 0:s.id)==="btn_send_transaction"&&u.can&&(await ft(u.token,u.amount)?document.getElementById("btn_send_transaction").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction").value="Ошибка при отправке транзакции"),((l=t.target)==null?void 0:l.id)==="btn_send_transaction_to_nft"&&v.can&&(await bt(v.amount)?document.getElementById("btn_send_transaction_to_nft").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_nft").value="Ошибка при отправке транзакции"),((i=t.target)==null?void 0:i.id)==="btn_send_transaction_to_sw"){let o=Number(localStorage.getItem("BIP_transfer_input_count"));o<=g.balance&&(await wt("UQB7q5qYhCOAIQbhWlIoq78ZXpGZWRn-gZc30-CSckZSMc_s",.05,o)?document.getElementById("btn_send_transaction_to_nft").value="Транзакция обрабатывается":document.getElementById("btn_send_transaction_to_nft").value="Ошибка при отправке транзакции")}if(((p=t.target)==null?void 0:p.id)==="copy_admin_card_number"){let o=document.getElementById("admin_card_number");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((c=t.target)==null?void 0:c.id)==="copy_card_send_code"){let o=document.getElementById("card_send_code");o.classList.add("anim"),setTimeout(()=>o.classList.remove("anim"),1e3),await navigator.clipboard.writeText(o.innerHTML)}if(((d=t.target)==null?void 0:d.classList.toString().split(" ").indexOf("BIP_NFT_sale_button"))!==-1)for(const o of k)o.index.toString()===t.target.id.split("_")[1]&&(console.log(o.address),console.log(k),await vt(o.address))})}let P=0;function ht(){if(Number(localStorage.getItem("BIP_balance"))>0){let t=document.getElementById("BIP_input_count");t==null||t.addEventListener("input",e=>{x(e.target.value,"BIP")}),!T&&t&&(t.type="text")}U()}function U(){var t,e,n,a,s;try{if(localStorage.getItem("BIP_balance")>0?((t=document.getElementById("BIP_input_count"))==null||t.addEventListener("input",l=>{It(l.target.value)}),(e=document.getElementById("changer_BIP2NFT"))==null||e.classList.remove("hide")):(n=document.getElementById("changer_BIP2NFT"))==null||n.classList.add("hide"),k.length!==0){let l=document.getElementById("wallet_nft_list"),i=1,p="";k.forEach(c=>{let d=c.metadata.name;d==="BIP NFT"&&(d=c.metadata.attributes[0].value.toString()+" "+d);let o=`BIPNFT_${c.index}`;p+=`
