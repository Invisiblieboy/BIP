...
export function walletInit() {
    ...
    if (tonConnectUI.connected) {
        ...
        if (Number(localStorage.getItem('BIP_balance'))>0 && params.beta==="true"){
    ...
    if (!value || value <= 0) {
        ...
        if (value * 1 <= coin_max) {
            if (btn_send_transaction) {
                ...
                btn_send_transaction.value = `Оплатить ${value} BIPCoin`
            }
...
function update_nft_img(bip_value=0) {
    ...
    if (classList.toString().split(' ').indexOf('hide')===-1){
        if ((bip_value>=10000) && (bip_value_last>=10000)){return;}
        if ((bip_value<=99) && (bip_value_last<=99)){return;}
    }
    bip_value_last = bip_value;

    axios.get(`https://nft.biptoken.xyz/CreateItemMetadata?value=${bip_value}`, {responseType: "json", transformResponse: (body) => body})
        .then((res) => {
            nft_img.src = JSON.parse(res.data).image
            nft_img.style.height = '5rem'
            nft_img.style.width = '5rem'
